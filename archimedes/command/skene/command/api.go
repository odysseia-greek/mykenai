...
func CreateApi() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "api",
		Short: "Create an api from templates",
		Long:  `Creates an api from templates.`,
		Run: func(cmd *cobra.Command, args []string) {
			...
			destinationPath, err := createScaffoldedService(name, indexName, repoName, destination, "", port, "api")
			if err != nil {
	...
	cmd.PersistentFlags().StringVarP(&port, "port", "p", "5000", "standard port for this service")
