...
const queryTemplate = `
query getDictionary($word: String!, $language: String!, $mode: String!, $searchInText: Boolean!) {
...
export default function () {
    ...
    let body;
    if (Math.random() < 0.75) { // 75% chance to use dictionary query
        const variables = queryVariables[Math.floor(Math.random() * queryVariables.length)];
        body = JSON.stringify({ query: queryTemplate, variables: variables });
    } else { // 25% chance to use a predefined query
        const query = predefinedQueries[Math.floor(Math.random() * predefinedQueries.length)];
